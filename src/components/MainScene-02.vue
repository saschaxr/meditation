...
export default defineComponent({
    ...
    methods: {
        ...
        async boot(): Promise<void> {
            ...
            BABYLON.SceneLoader.ImportMesh("","https://ams3.digitaloceanspaces.com/doob-highfidelity/MeditationSpaceLarger/NPC/","Priscilla_Talking01.glb", scene._scene, (meshes, particleSystems, skeletons, animationGroups) => {
                console.log("Gefundene AnimationGroups:", animationGroups.map((a) => a.name));
                // Animation starten (Idle oder Talking)
                const talkingAnim = animationGroups.find((a) => a.name.toLowerCase().includes("talk"));
                if (talkingAnim) {
                    talkingAnim.play(true); // true = loop
                    console.log("Talking Animation gestartet");
                } else if (animationGroups.length > 0) {
                    // fallback: erste Animation startet
                    animationGroups[0].play(true);
                    console.log("Fallback Animation gestartet");
                }
            });
