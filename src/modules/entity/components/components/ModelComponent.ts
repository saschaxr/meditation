        ...
        void SceneLoader.ImportMeshAsync(
            ...
            .then((result) => {
                ...
                if (entity.animation && result.animationGroups) {
                    ...
                    //This is a map of all animated meshes in the scene. Each mesh name is associated with its AnimationGroup. 
                    //In renderer.ts, the createScene method uses this map to find and trigger the correct animation when a mesh is clicked.
                    //animInfo take data from Json for animation loop
                    const animInfo = {
                        loop: entity.animation?.loop ?? true}

                    const animationMap = new Map<string, { anim: AnimationGroup; info: typeof animInfo }>();
                    result.animationGroups.forEach((ag) => {
                        animationMap.set(ag.name, { anim: ag, info: animInfo });
                    });

                    // Store it on the scene so renderer can access it
                    (scene as any).animationMap = animationMap;            
